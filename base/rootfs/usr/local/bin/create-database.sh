...
function cmdline {
    ...
    case ${DRIVER} in
        mysql|pdo_mysql)
            if fallback "--host" "HOST" "DB_MYSQL_HOST"; then
                readonly HOST=${DB_MYSQL_HOST}
            fi
            if fallback "--port" "PORT" "DB_MYSQL_PORT"; then
                readonly PORT=${DB_MYSQL_PORT}
            fi
            ;;
        pgsql|postgresql|pdo_pgsql)
            if fallback "--host" "HOST" "DB_POSTGRESQL_HOST"; then
                readonly HOST=${DB_POSTGRESQL_HOST}
            fi
            if fallback "--port" "PORT" "DB_POSTGRESQL_PORT"; then
                readonly PORT=${DB_POSTGRESQL_PORT}
            fi
            ;;
        *)
