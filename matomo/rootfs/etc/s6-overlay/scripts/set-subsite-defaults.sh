...
{
    for MATOMO_SITE in ${MATOMO_SUBSITES}; do
        for FILE in /var/run/s6/container_environment/MATOMO_DEFAULT_*; do
        ...
        for FILE in /var/run/s6/container_environment/MATOMO_USER_*; do
            DEFAULT_VAR=$(basename "${FILE}")
